...
export function EnvironmentForm({
  ...
  formData: IEnvsItem;
  onChange: (data: IEnvsItem) => void;
}) {
  const { list } = useSelector('ssh');

  const onChangeValue = useCallback(
    <T extends keyof IEnvsItem>(name: T, value: IEnvsItem[T]) => {
      onChange({
        ...formData,
        [name]: value
      });
  ...
  return (
    <>
      ...
      <Form.Item label="服务器SSH">
        <Select
          value={formData.ssh_id}
          onChange={value => onChangeValue('ssh_id', value)}
          style={{ width: 120 }}
        >
          {list.map(item => (
            <Option key={item.ssh_id} value={item.ssh_id}>
              {item.name}
            </Option>
          ))}
        </Select>
